                ...
                <div class="divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    {{-- <div class="flex flex-wrap items-center gap-y-4 py-6">
                        ...
                        <div class="w-full grid sm:grid-cols-2 lg:flex lg:w-64 lg:items-center lg:justify-end gap-4">
                            <button disabled type="button" class="w-full rounded-lg border border-red-700 px-3 py-2 text-center text-sm font-medium text-red-700 hover:bg-red-700 hover:text-white focus:outline-none focus:ring-4 focus:ring-red-300 dark:border-red-500 dark:text-red-500 dark:hover:bg-red-600 dark:hover:text-white dark:focus:ring-red-900 lg:w-auto">Cancel order</button>
                    ...
                    @foreach ($orders as $key => $order)
                        <div class="flex flex-wrap items-center gap-y-4 py-6">
                            ...
                            <dl class="w-1/2 sm:w-1/4 lg:w-auto lg:flex-1">
                                <dt class="text-base font-medium text-gray-500 dark:text-gray-400">Car Model</dt>
                                <dd class="mt-1.5 text-base font-semibold text-gray-900 dark:text-white">{{ $order->cars->cars_model}}</dd>
                            </dl>

                            <dl class="w-1/2 sm:w-1/4 lg:w-auto lg:flex-1">
                                <dt class="text-base font-medium text-gray-500 dark:text-gray-400">Lease Start</dt>
                                <dd class="mt-1.5 text-base font-semibold text-gray-900 dark:text-white">{{ Carbon::parse($order['order_starts'])->format('Y-m-d H:i:s') }}</dd>
                            </dl>
                            <dl class="w-1/2 sm:w-1/4 lg:w-auto lg:flex-1">
                                <dt class="text-base font-medium text-gray-500 dark:text-gray-400">Lease Ends</dt>
                                <dd class="mt-1.5 text-base font-semibold text-gray-900 dark:text-white">{{ Carbon::parse($order['order_ends'])->format('Y-m-d H:i:s') }}</dd>
                            </dl>

                            <dl class="w-1/2 sm:w-1/4 lg:w-auto lg:flex-1">
                                <dt class="text-base font-medium text-gray-500 dark:text-gray-400">Price /day:</dt>
                                <dd class="mt-1.5 text-base font-semibold text-gray-900 dark:text-white">@money($order->cars->cars_price)</dd>
                            </dl>

                            <dl class="w-1/2 sm:w-1/4 lg:w-auto lg:flex-1">
                                <dt class="text-base font-medium text-gray-500 dark:text-gray-400">Status:</dt>
                                <dd class="me-2 mt-1.5 inline-flex items-center rounded 
                                    @if($order->order_status === 'disewa')
                                        bg-blue-100 px-2.5 py-0.5 text-xs font-medium text-blue-800 dark:bg-blue-900 dark:text-blue-300
                                    @elseif($order->order_status === 'diperiksa')
                                        bg-green-100 px-2.5 py-0.5 text-xs font-medium text-yellow-800 dark:bg-yellow-900 dark:text-yellow-300
                                    @elseif($order->order_status === 'selesai')
                                        bg-green-100 px-2.5 py-0.5 text-xs font-medium text-green-800 dark:bg-green-900 dark:text-green-300
                                    @elseif($order->order_status === 'denda')
                                        bg-red-100 px-2.5 py-0.5 text-xs font-medium text-red-800 dark:bg-red-900 dark:text-red-300
                                    @endif">
                            ...
                            <div class="w-full grid sm:grid-cols-2 lg:flex lg:w-64 lg:items-center lg:justify-end gap-4">
                                @if($order->order_status === 'disewa')
                                    <button data-modal-target="crud-modal{{ $order->id }}" data-modal-toggle="crud-modal{{ $order->id }}" type="button" class="w-full rounded-lg border border-red-700 px-3 py-2 text-center text-sm font-medium text-red-700 hover:bg-red-700 hover:text-white focus:outline-none focus:ring-4 focus:ring-red-300 dark:border-red-500 dark:text-red-500 dark:hover:bg-red-600 dark:hover:text-white dark:focus:ring-red-900 lg:w-auto">Return a car</button>
                                @elseif($order->order_status === 'selesai')
                                    <button type="button" class="w-full rounded-lg border border-green-700 px-3 py-2 text-center text-sm font-medium text-green-700 hover:bg-green-700 hover:text-white focus:outline-none focus:ring-4 focus:ring-green-300 dark:border-green-500 dark:text-green-500 dark:hover:bg-green-600 dark:hover:text-white dark:focus:ring-green-900 lg:w-auto">Order again</button>
                                @elseif($order->order_status === 'diperiksa')
                                    <button type="button" class="w-full rounded-lg border border-gray-700 px-3 py-2 text-center text-sm font-medium text-gray-700 hover:bg-gray-700 hover:text-white focus:outline-none focus:ring-4 focus:ring-gray-300 dark:border-gray-500 dark:text-gray-500 dark:hover:bg-gray-600 dark:hover:text-white dark:focus:ring-gray-900 lg:w-auto cursor-not-allowed" disabled>Return a car</button>
                                @endif
                                <a href="{{ route('orderDetail', ['id' => $order->id])}}" class="w-full inline-flex justify-center rounded-lg  border border-gray-200 bg-white px-3 py-2 text-sm font-medium text-gray-900 hover:bg-gray-100 hover:text-primary-700 focus:z-10 focus:outline-none focus:ring-4 focus:ring-gray-100 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white dark:focus:ring-gray-700 lg:w-auto">View details</a>
                            </div>
                        </div>

                        <!-- Main modal -->
                        <div id="crud-modal{{ $order->id }}" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                            <div class="relative p-4 w-full max-w-md max-h-full">
                                ...
                                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                    ...
                                    <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                                        ...
                                        <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-toggle="crud-modal{{ $order->id }}">
                                    ...
                                    <!-- Modal body -->
                                    <form action="{{ route('returnCar') }}" method="POST" class="p-4 md:p-5">
                                        @csrf
                                        @method('put')
                                        <div class="grid gap-4 mb-4 grid-cols-2">
                                            <div class="col-span-2">
                                                ...
                                                <input type="text" name="cars_nopol" id="cars_nopol" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="XX 1234 XX" required="">
