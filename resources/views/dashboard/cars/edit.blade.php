...
@section('content')
    <div class="relative overflow-x-auto sm:rounded-lg max-w-2xl">
        <h2 class="mb-4 text-xl font-bold text-gray-900 dark:text-white">Edit Car</h2>
        <form action="{{ route('updateCar', ['id' => $car->id]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                ...
                <div class="sm:col-span-2">
                    ...
                    @if ($car->cars_image)
                        <img src="{{ asset('storage/uploads/' . $car->cars_image) }}" alt="Car Image" class="mb-2 max-w-full h-auto">
                    @endif
                ...
                <div class="sm:col-span-2">
                    ...
                    <select id="category_id" name="category_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                        ...
                        @foreach ($category as $id => $cat)
                            <option value="{{ $id }}" {{ $id == $car->category_id ? 'selected' : '' }}>
                                {{ $cat }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="sm:col-span-2">
                    ...
                    <input id="cars_description" type="hidden" name="cars_description" value="{{ old('deskripsi_novel', $car->cars_description ?? '') }}">
